package {
	public class md53 {
		...
		public function md53(str:String, sysId:String) {
			hex_chr = sysId+"zxcvbnm,.asdfghjkl;'qwertyuiop[]"
			var xx = str2blks_MD5(str);
			var a = 1732584193;
			var b = -271733879;
			var c = -1732584194;
			var d = 271733878;

			for (var i=0; i<xx.length; i += 16) {
				var olda = a;
				var oldb = b;
				var oldc = c;
				var oldd = d;
		...
		private function rhex(num) {
			str = "";
			for (var j=0; j<=3; j++) {
				var tempAnd1 = bitAND((num >> (j*8+4)), 0x0F);
				var tempAnd2 = bitAND((num >> (j*8)), 0x0F);
		...
		private function addme(x, y) {
			var tempAnd3 = bitAND(x, 0xFFFF);
			var tempAnd4 = bitAND(y, 0xFFFF);
			var lsw = tempAnd3+tempAnd4;
			var msw = (x >> 16)+(y >> 16)+(lsw >> 16);
			var tempAnd9 = bitAND(lsw, 0xFFFF);
		...
		private function ff(a, b, c, d, x, s, t) {
			var tempAnd5 = bitAND(b, c);
			var tempAnd6 = bitAND(~b, d);
			return cmn(tempAnd5 | tempAnd6,a,b,x,s,t);
		}
		private function gg(a, b, c, d, x, s, t) {
			var tempAnd7 = bitAND(b, d);
			var tempAnd8 = bitAND(c, ~d);
